...
fn tester(index: usize) -> usize {
    ...
    // let mut result_file = File::create(&format!("{}_out.txt", index)).unwrap();
    // result_file
    //     .write_all(format!("{}\n# score = {}\n", ans, score).as_bytes())
    //     .unwrap();
    score
...
fn main() {
    ...
    eprintln!("# average score: {}", sum / 100);
    return;
    input! {
        n: usize,
        d: usize,
    ...
    let max_div_num = if d / 2 > 1 { d } else { 2 };
    let mut search_min_num = if 8 < q / n { 8 } else { q / n };
    ...
    let mut query_cnt = 0;
    let mut div_num = 2;
    let search_min_or_max_num = if 4 < q / n { 4 } else { q / n };
    ...
    println!("# score: {}", calc_score(&ans, &ws, d));
    io::stdout().flush().unwrap();
