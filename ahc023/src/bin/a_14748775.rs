...
fn dfs(
    ...
    for dir in 0..4 {
        ...
        if DY[dir] == 1 && i < height - 1 && h[i][j] == '1' {
            continue;
        }
        if DY[dir] == -1 && i >= 1 && h[i - 1][j] == '1' {
            continue;
        }
        if DX[dir] == 1 && j < width - 1 && v[i][j] == '1' {
            continue;
        }
        if DX[dir] == -1 && j >= 1 && v[i][j - 1] == '1' {
            continue;
        }
        if visited[ni as usize][nj as usize] {
            continue;
        }
        if used[ni as usize][nj as usize] {
            continue;
        }
        dfs(ni as usize, nj as usize, h, v, height, width, visited, used);
    }
}
...
fn path_exists(
    ...
    tmp_used[gi][gj] = false;
    dfs(si, sj, h, v, height, width, &mut visited, &tmp_used);
    visited[gi][gj]
}
